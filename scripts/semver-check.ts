 ...
 */
function getVersionBumpType(fromVersion: string, toVersion: string): 'major' | 'minor' | 'patch' | 'none' {
 ...
 */
function readDiffClassification(classificationPath: string): DiffClassification {
 ...
 */
function getLastVersionFromChangelog(changelogPath: string = 'CHANGELOG.md'): string | null {
...
function validateSemverAlignment(
  ...
  console.log(`📈 Version bump: ${fromVersion} → ${toVersion} (${actualBump.toUpperCase()})`);
...
function performSemverCheck(options: {
  ...
  const { classificationPath, packageJsonPath = 'package.json', changelogPath = 'CHANGELOG.md', verbose = false } = options;
  ...
  try {
    ...
    if (!previousVersion) {
      console.log('ℹ️ No previous version found in changelog, skipping semver check');
      return;
    }
    ...
    // Validate alignment
    const validation = validateSemverAlignment(classification, previousVersion, currentVersion);
    ...
    if (!validation.valid) {
      ...
      if (classification.classification === 'major') {
        ...
        console.error('   - For NON_BREAKING (additive) changes, increment minor version');
        console.error('   - Example: 1.2.3 → 1.3.0');
      } else if (classification.classification === 'patch') {
        console.error('   - For PATCH (documentation/examples) changes, increment patch version');
    ...
    if (verbose) {
      console.log(`✨ Version bump ${previousVersion} → ${currentVersion} is appropriate for ${classification.classification.toUpperCase()} changes`);
    }
