...
async function main() {
  try {
    ...
    if (!repoInfo) {
      console.error(
        "Error: Could not determine repository information from GITHUB_REPOSITORY",
      );
      process.exit(1);
    }
    ...
    if (!prNumber) {
      ...
      console.error(
        "Set GITHUB_PR_NUMBER, PR_NUMBER, or PULL_REQUEST_NUMBER environment variable",
      );
      process.exit(1);
    }
    ...
    // Check if PR already has the label
    const hasLabel = await prHasLabel(
      repoInfo.owner,
      repoInfo.repo,
      prNumber,
      labelName,
    ...
    const added = await addLabelToPR(
      repoInfo.owner,
      repoInfo.repo,
      prNumber,
      labelName,
    ...
    if (added) {
      console.log(
        `✅ Successfully added '${labelName}' label to PR #${prNumber}`,
      );
