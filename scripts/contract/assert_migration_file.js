...
function validateMigrationContent(content, classification) {
  ...
  if (classification.semanticChange === "major") {
    if (
      ...
      !contentLower.includes("backward incompatible")
    ) {
  ...
  const hasMigrationGuidance = migrationKeywords.some((keyword) =>
...
`;
  ...
  if (
    classification.addedEndpoints &&
    classification.addedEndpoints.length > 0
  ) {
...
`;
  ...
  if (
    classification.removedEndpoints &&
    classification.removedEndpoints.length > 0
  ) {
...
async function main() {
  try {
    ...
    if (!classification) {
      ...
      console.error(
        "Provide classification file path or set DIFF_CLASSIFICATION environment variable",
      );
      process.exit(1);
    }
    ...
    console.log(
      `${classification.semanticChange.toUpperCase()} version change detected - checking for migration files...`,
    );
    ...
    if (migrationFiles.length === 0) {
      console.error(
        `❌ No migration files found in ${migrationsDir}/ directory`,
      );
      console.error(
        `${classification.semanticChange.toUpperCase()} changes require migration documentation`,
      );
      ...
      const version =
        process.env.CONTRACT_VERSION || new Date().toISOString().split("T")[0];
      ...
      console.error(
        `Please review and complete the migration documentation before proceeding`,
      );
      process.exit(1);
    }
    ...
    for (const migrationFile of migrationFiles) {
      if (migrationFile.content) {
        const validation = validateMigrationContent(
    ...
    if (hasValidMigration) {
      console.log(
        `✅ Valid migration documentation found in ${migrationsDir}/`,
      );
      ...
      console.error(
        `❌ No valid migration documentation found in ${migrationsDir}/`,
      );
