...
async function deployOpenAPISpec() {
  try {
    ...
    if (!openApiSpec.openapi) {
      throw new Error(
        'OpenAPI specification is missing the required "openapi" version field',
      );
    }
    ...
    // Upload to S3
    console.log(
      `Uploading OpenAPI specification to s3://${s3Bucket}/${s3Path}...`,
    );
    execSync(
      `aws s3 cp ${sourceFile} s3://${s3Bucket}/${s3Path} --content-type application/json`,
    );

    // Invalidate CloudFront cache
    console.log("Invalidating CloudFront cache...");
    execSync(
      `aws cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/${s3Path}"`,
    );

    console.log("OpenAPI specification deployment completed successfully");
    console.log(
      `The OpenAPI specification is now available at https://openapi.bondmcp.com/openapi.bondmcp`,
    );
