...
describe("BondMCP Integration Tests", () => {
  ...
  it("should handle rate limiting gracefully", async () => {
    const promises = Array(10)
      .fill()
      .map(() => client.health.ask({ question: "Test question" }));
  ...
  it("should validate required parameters", async () => {
    await expect(client.health.ask({ question: "" })).rejects.toThrow();
  });
});
```
...
const client = new MonitoredBondMCPClient(process.env.BONDMCP_API_KEY, logger);
```
...
_SDK Examples last updated: August 4, 2025_  
_SDK Versions: Python 1.2.0, JavaScript 1.1.0, Go 1.0.0_
