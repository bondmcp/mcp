...
describe("Contract Ingest Scripts", () => {
  ...
  test("normalize_spec.ts should normalize OpenAPI spec", async () => {
    ...
    // Run normalization
    execSync(
      `npx ts-node scripts/contract/normalize_spec.ts --in ${inputFile} --out ${outputFile}`,
      {
        cwd: process.cwd(),
      },
    );
    ...
    strictEqual(
      normalized.info.version,
      "1.0.0",
      "Version timestamp should be removed",
    );
    strictEqual(
      normalized.openapi,
      "3.0.2",
      "OpenAPI version should be preserved",
    );
  ...
  test("publish_preflight.ts should detect existing package", async (t) => {
    try {
      ...
      if (error.status === 20) {
        ...
        console.log(
          "Note: npm check may have failed due to network or other issues",
        );
      }
    }
  });
  ...
  test("assert_migration_file.ts should handle missing classification", async () => {
    ...
    try {
      execSync(
        "npx ts-node scripts/contract/assert_migration_file.ts --classification /tmp/nonexistent.json",
        {
          cwd: process.cwd(),
          stdio: "pipe",
        },
  ...
  test("package.json scripts should be available", async () => {
    ...
    ok(
      packageJson.scripts["contract:normalize"],
      "normalize script should exist",
    );
    ok(
      packageJson.scripts["contract:preflight"],
      "preflight script should exist",
    );
